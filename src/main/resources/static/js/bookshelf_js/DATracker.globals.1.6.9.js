(function () {
    var Config = {DEBUG: false, LIB_VERSION: "1.6.9"};
    var _$1 = {
    ...
    if (typeof window === "undefined") {
        win$1 = {navigator: {userAgent: ""}, document: {}}
    } else {
        win$1 = window
    }
    var device;
    ...
    }], ["pc", "windows"], ["ipad", "ipad"], ["ipod", "ipod"], ["iphone", /\biphone\b|\biph(\d)/], ["mac", "macintosh"], ["mi", /\bmi[ \-]?([a-z0-9 ]+(?= build|\)))/], ["hongmi", /\bhm\b|redmi[ \-]?([a-z0-9]+)/], ["aliyun", /\baliyunos\b(?:[\-](\d+))?/], ["meizu", function (ua) {
    ...
    function detect(name, expression, ua) {
        ...
        var info = {name: name, version: NA_VERSION, codename: ""};
        var t = toString(expr);
        if (expr === true) {
            return info
        } else {
    ...
    _.dom_query = function () {
        ...
        return function (query) {
            if (_.isElement(query)) {
                ...
                if (_.isObject(query) && !_.isUndefined(query.length)) {
                    return query
                } else {
    ...
    _.getEleInfo = function (obj, heatmapConfig, context) {
        ...
        if (tagName === "input") {
            if (target.type === "button" || target.type === "submit") {
                ...
                if (heatmapConfig && typeof heatmapConfig.collect_input === "function" && heatmapConfig.collect_input(target, context)) {
                    props.text = target.value || ""
                }
            }
        }
    ...
    var campaign = {
        data: {
            campaign: {
                ...
                utm_term: "",
                promotional_id: ""
            }, isCampaign: false, isAdClick: false, campaignParamsSaved: false, APPKEY: ""
    ...
    var single_page = {
        config: {
            mode: "hash", track_replace_state: false, callback_fn: function () {
            }
    ...
    var source = {
        data: {secondLevelSource: "", outsideReferer: false, APPKEY: ""}, init: function (APPKEY) {
            if (typeof APPKEY === "undefined") {
                return
            }
            this.data.APPKEY = APPKEY;
    ...
    var heatmap = {
        getDomIndex: function (el) {
            ...
            return {x: isNaN(b) ? 0 : b, y: isNaN(a) ? 0 : a}
            ...
            function linkFunc(e) {
                ...
                that.DATracker.track(eventId, eventProp, track_a_click, "auto")
            }
            ...
            if (heatmapConfig.collect_elements === "all") {
                ...
                _.addEvent(document, "click", function (e) {
                    ...
                    if (tagName === "a" || tagName === "button" || tagName === "input" || tagName === "textarea") {
                        ...
                        if (parent_ele.tagName.toLowerCase() === "button" || parent_ele.tagName.toLowerCase() === "a") {
                            that.start(ev, parent_ele, target.parentNode.tagName.toLowerCase())
                        } else {
                            if (tagName === "area" && parent_ele.tagName.toLowerCase() === "map" && _.ry(parent_ele).prev().tagName && _.ry(parent_ele).prev().tagName.toLowerCase() === "img") {
            ...
            function isReady(data) {
                if (heatmap_url) {
                    _.loadScript({
                        ...
                        }, type: "js", url: heatmap_url + "?_=" + Math.random()
                    })
                } else {
            ...
            if (match && match[0] && match[1]) {
                ...
                if (_.sessionStorage.isSupport() && typeof sessionStorage.getItem("hubble_heatmap_id") === "string") {
                    me.DATracker.set_config({hubble_render_mode: true});
    ...
    var debug = {
        _queue: [], _callbackList: [], _loadControlComplete: false, init: function (instance) {
            if (!instance) {
                return
            }
            var that = this;
                    ...
                    that._prepare(function () {
                        if (typeof hubbleData_render_mode_fn === "function") {
                            that._control = new hubbleData_render_mode_fn(that.DATracker, {
                                type: "abtest_debug",
                                data: {hubble_abtest_debug_key: that.getDebugKeyData()}
                            }).control;
            ...
            if (match && match[0] && match[1]) {
                ...
                if (_.sessionStorage.isSupport() && typeof sessionStorage.getItem("hubble_abtest_debug_key") === "string") {
            ...
            if (control_js_url) {
                _.loadScript({
                    ...
                    }, type: "js", url: control_js_url + "?_=" + Math.random()
                })
            } else {
    ...
    var multilinkAbest = {
        multilinkArr: [], isTestDebug: debug._isTestDebug, setMultilinkArr: function (multilinkArr) {
    ...
    var visualizationAbest = {
        _visualizationArr: [], _settimeNum: 0, _loadControlJs: function (callback) {
            var that = this;
            var control_js_url = that.DATracker.config.control_js_url;
            if (control_js_url) {
                _.loadScript({
        ...
        }, isTestDebug: debug._isTestDebug, setVisualizationArr: function (visualizationArr) {
    ...
    var abtest = {
                        ...
                        that.get_variation(function (abtest) {
                            try {
                                ...
                                for (var j = 0; j < nowVisualizationVariableArr.length; j += 1) {
                                    ...
                                    abtest.get(item.variable, {}, function () {
                                    }, {variableObj: item})
                                }
                            } catch (error) {
                                console.error(error)
        ...
        _clearLocalData: function () {
            this.data = {experiments: [], variables: {}, updatedTime: 0};
            this._saveLocal()
        },
        ...
        _variableToFindExperiment: function (variable) {
            var dataObj = {$experimentId: [], $versionId: [], $experimentType: this._getExperimentType(variable)};
    ...
    var DEFAULT_CONFIG = {
        ...
        "single_page_config": {mode: "hash", track_replace_state: false},
        "pageview": true,
        "use_app_track": false,
    ...
    var DATATYPE = "e";
    var DEFAULTEVENTID = {
        "da_session_start": {"dataType": "ie"},
        "da_session_close": {"dataType": "ie"},
        "da_u_login": {"dataType": "ie"},
        "da_u_logout": {"dataType": "ie"},
        "da_u_signup": {"dataType": "ie"},
        "da_user_profile": {"dataType": "ie"},
        "da_screen": {"dataType": "pv"},
        "da_ad_click": {"dataType": "ie"},
        "da_activate": {"dataType": "ie"},
        "da_abtest": {"dataType": "ie"},
        "da_send_error": {"dataType": "ie"}
    };
    ...
    var DATrackerPersistence = function (config) {
        ...
        var localStorage_supported = function () {
            var supported = true;
            try {
                var key = "__hbssupport__", val = "xyz";
    ...
    DATrackerPersistence.prototype._add_to_people_queue = function (queue, data) {
        var q_key = this._get_queue_key(queue), q_data = data[queue], set_q = this._get_or_create_queue(SET_ACTION),
            set_once_q = this._get_or_create_queue(SET_ONCE_ACTION), add_q = this._get_or_create_queue(ADD_ACTION),
            union_q = this._get_or_create_queue(UNION_ACTION), append_q = this._get_or_create_queue(APPEND_ACTION, []);
        if (q_key === SET_QUEUE_KEY) {
            _.extend(set_q, q_data);
            this._pop_from_people_queue(ADD_ACTION, q_data);
            this._pop_from_people_queue(UNION_ACTION, q_data)
        } else {
            if (q_key === SET_ONCE_QUEUE_KEY) {
    ...
    DATrackerPersistence.prototype._get_queue_key = function (queue) {
        if (queue === SET_ACTION) {
            ...
            if (queue === SET_ONCE_ACTION) {
                return SET_ONCE_QUEUE_KEY
            } else {
                if (queue === ADD_ACTION) {
                    return ADD_QUEUE_KEY
                } else {
                    if (queue === APPEND_ACTION) {
                        return APPEND_QUEUE_KEY
                    } else {
                        if (queue === UNION_ACTION) {
    ...
    var create_DAlib = function (token, config, name) {
        var instance, target = name === PRIMARY_INSTANCE_NAME ? DATracker_master : DATracker_master[name];
    ...
    DATrackerLib.prototype._init = function (token, config, name) {
        ...
        heatmap.init(_self, function () {
            ...
            if (_self.get_config("is_single_page")) {
                _self["cookie"].register({currentReferrer: location.href});
                _self._single_page()
            }
    ...
    DATrackerLib.prototype._session = function (pvCallback) {
        ...
        this["cookie"].register({updatedTime: nowDateTime});
    ...
    DATrackerLib.prototype._single_page = function () {
        ...
        if (this.get_config("single_page_config").mode === "hash") {
            callback_fn = function () {
                _self["cookie"].register({sessionReferrer: current_page_url});
                _self._single_pageview();
                current_page_url = location.href;
                change()
            }
        } else {
            if (this.get_config("single_page_config").mode === "history") {
                callback_fn = function () {
                    _self._single_pageview();
                    change()
                }
    ...
    DATrackerLib.prototype._send_request = function (url, data, callback, errorFn, otherFn) {
        ...
        if (!USE_XHR) {
            if (callback) {
                ...
                if (verbose_mode || this.get_config("test")) {
        ...
        if ("img" in data) {
            ...
            if (USE_XHR) {
                try {
                    ...
                    req.onreadystatechange = function () {
                        if (req.readyState === 4) {
                            ...
                            if (req.status === 200) {
                                ...
                                if (errorFn) {
                                    errorFn({state: req.status, statusText: req.statusText})
                                }
                            }
                        }
                    };
                    ...
                    if (errorFn) {
                        errorFn({state: "notSend", statusText: e})
                    }
                }
            } else {
        ...
        _.each(array, function (item) {
            if (item) {
                ...
                if (typeof item === "function") {
                    ...
                    if (_.isArray(item) && fn_name === "alias") {
                        alias_calls.push(item)
                    } else {
                        if (_.isArray(item) && fn_name.indexOf("track") !== -1 && typeof this[fn_name] === "function") {
    ...
    DATrackerLib.prototype.track = function (event_name, properties, callback, setData_type) {
        ...
        if (data_type === "e") {
            if (!this._check_referer()) {
                this["cookie"].register({sessionReferrer: location.href})
            }
        }
        ...
        var jsTrack = function () {
            self._send_request(self.get_config("api_host") + "/track/w/", {
                "data": encoded_data,
                "appKey": appkey_data
            }, self._prepare_callback(callback, truncated_data), errorFn, otherFn)
        };
        if (this.get_config("use_app_track")) {
        ...
        if (!_.include(["da_session_close", "da_session_start"], event_name)) {
            this["cookie"].register({LASTEVENT: {eventId: event_name, time: time}})
        }
    ...
    DATrackerLib.prototype._single_pageview = function () {
        ...
        if (this.get_config("is_single_page")) {
            if (this.get_config("single_page_config").mode === "hash" || this.get_config("single_page_config").mode === "history" || this.get_config("single_page_config").mode === "memoryhistory") {
                this["cookie"].register({sessionReferrer: currentReferrer, currentReferrer: current_page_url})
            }
    ...
    DATrackerLib.prototype.signup = function (user_id) {
        ...
        if (oldUserId == user_id) {
            ...
            data = this.track("da_u_signup", {"oldUserId": oldUserId, "newUserId": user_id})
        }
        return data
    };
    DATrackerLib.prototype.login = function (user_id) {
        ...
        this._register_single("user_id", user_id);
        return {data_signup: data_signup, data_login: data_login}
    };
    ...
    DATrackerPeople.prototype.set_populationWithAccount = function (account, realname, birthday, gender) {
        if (!account || !realname || !birthday || !gender) {
            return
        }
        if (!_.checkTime(birthday)) {
            return
    ...
    DATrackerPeople.prototype._send_request = function (data, callback) {
        ...
        var self = this;
        var jsTrack = function () {
            self._DATracker._send_request(self._get_config("api_host") + "/track/w/", {
                "data": encoded_data,
                "appKey": appkey_data
